...
[task_local]
#jd免费水果
20 8,15 * * * jd_fruit.js, tag=东东农场, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdnc.png, enabled=true
=========================Loon=============================
[Script]
cron "20 8,15 * * *" script-path=jd_fruit.js,tag=东东农场

=========================Surge============================
东东农场 = type=cron,cronexp="20 8,15 * * *",wake-system=1,timeout=3600,script-path=jd_fruit.js

=========================小火箭===========================
东东农场 = type=cron,script-path=jd_fruit.js, cronexpr="20 8,15 * * *", timeout=3600, enable=true
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://transfer12.nz.lu/farm`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
