...
if [[ -z "$(echo "$DIR"|grep 'main')" ]];then
    dir_code=$dir_log/lichen780_JD_jd_sharecode
    repo='lichen780_JD' 
else
    dir_code=$dir_log/lichen780_JD_main_jd_sharecode
    repo='lichen780_JD_main' 
fi
...
[[ $AUTOCFG == true ]] && cp $dir_repo/lichen780_JD/sendNotify.js $dir_deps/ > /dev/null 2>&1
